...
myPWMServoDriver::myPWMServoDriver(uint8_t addr) {
  _i2caddr = addr;
}

void myPWMServoDriver::begin(void) {
 WIRE.begin();
 reset();
}


void myPWMServoDriver::reset(void) {
 write8(PCA9685_MODE1, 0x0);
}

void myPWMServoDriver::setPWMFreq(float freq) {
...
void myPWMServoDriver::setPWM(uint8_t num, uint16_t on, uint16_t off) {
...
void myPWMServoDriver::setPin(uint8_t num, uint16_t val, bool invert)
{
...
uint8_t myPWMServoDriver::read8(uint8_t addr) {
  WIRE.beginTransmission(_i2caddr);
  WIRE.write(addr);
  WIRE.endTransmission();
...
void myPWMServoDriver::write8(uint8_t addr, uint8_t d) {
  WIRE.beginTransmission(_i2caddr);
  WIRE.write(addr);
  WIRE.write(d);
  WIRE.endTransmission();
